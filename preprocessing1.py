...
class RaceDataPreprocessor1:
    ...
    def _add_jockey_features(self, df):
        ...
        df["騎手の平均人気"] = df["騎手"].map(lambda x: self.jockey_stats.get(x, {}).get("平均人気", 0))
    ...
    def _common_preprocessing(self, df):
        ...
        # 不要な列を削除
        df = df.drop(columns=["レース名", "場id", "通過順", "開催","走破時間", "上がり","着順"], errors="ignore")
        # 日本語の日付フォーマットを指定して変換
    ...
    def _scale_numeric(self, df):
        df["馬の出走回数"] = np.log1p(df["馬の出走回数"])
        df["騎手の出走回数"] = np.log1p(df["騎手の出走回数"])
        
        numeric_cols = ["体重", "体重変化", "斤量", "人気", "オッズ", 
                        "馬の平均人気", "馬の平均着順", "馬の出走回数",
                        "騎手の平均人気", "騎手の平均着順", "騎手の出走回数", "距離"]
